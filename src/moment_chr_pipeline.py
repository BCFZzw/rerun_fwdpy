...
def record_moments_LD(zarr_path, window_list, threshold = 0, panel_file = None, super_pop = None, pop = None):
    ...
    for i in range(n_windows):
        ...
        for arr, stats in zip([D2_filter_list, Dz_filter_list, D_filter_list, pi2_filter_list], [D2_pw, Dz_pw, D_pw, pi2_pw]):
            ### if NaN cases present, the sum will be NaN
            arr[i] = np.sum(stats)
        pair_count_filter_list[i] = len(D2_pw)
    

    LD_filtered_dict = {"D2_pw": D2_filter_list, "Dz_pw": Dz_filter_list, 
    "D_pw": D_filter_list, "pi2_pw": pi2_filter_list, "pair_count": pair_count_filter_list, "windows": window_list}

    return LD_filtered_dict